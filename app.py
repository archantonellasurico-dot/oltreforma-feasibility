...
def merge_tax_relief_defaults(p):
    ...
    # V3.6 · Sismabonus acquisti come leva commerciale / scenario ricavi
    sbc = i.setdefault("sismabonus_acquisti_commerciale", {})
    sbc.setdefault("attivo", False)
    sbc.setdefault("requisiti_verificati", False)
    sbc.setdefault("vendita_entro_30_mesi", False)
    sbc.setdefault("zona_sismica", 0)
    sbc.setdefault("unita_agevolabili", 0)  # 0 = usa automaticamente il numero alloggi
    sbc.setdefault("prezzo_medio_unitario_manuale", 0.0)  # 0 = ricava dai ricavi di scenario
    sbc.setdefault("massimale_per_unita_euro", 96000.0)
    sbc.setdefault("aliquota_ordinaria_pct", 36.0)
    sbc.setdefault("aliquota_prima_casa_pct", 50.0)
    sbc.setdefault("quota_prima_casa_pct", 0.0)
    sbc.setdefault("quota_beneficio_capitalizzabile_pct", 0.0)
    sbc.setdefault("includi_premio_nei_kpi", False)
    i.setdefault("itaca_attivo", False)
    i.setdefault("itaca_pct", 0.0)
    i.setdefault("itaca_stato", "Da verificare")
    i.setdefault("itaca_riferimento", "")
    i.setdefault("romani_attivo", False)
    i.setdefault("romani_pct", 0.0)
    i.setdefault("romani_stato", "Da verificare")
    i.setdefault("romani_riferimento", "")
    i.setdefault("base_premialita", "Volume esistente legittimo")
    i.setdefault("cumulabilita_itaca_romani_verificata", False)
    return p
...
def ensure_session():
    ...
    if "project_selector" not in st.session_state:
        st.session_state.project_selector = st.session_state.current_project
    if "view_mode" not in st.session_state:
        st.session_state.view_mode = "Vista Studio"


def clear_project_widget_state():
    """Evita che campi con key esplicite del progetto precedente contaminino quello appena aperto."""
    prefixes = ("mix_", "fee_")
...
def select_current_project():
    ...
    if selected in st.session_state.projects:
        ...
        clear_project_widget_state()


def rename_current_project(new_name):
    old_name = st.session_state.current_project
    new_name = (new_name or "").strip()
    if not old_name or not new_name:
        return False, "Inserisci un nome."
    if new_name == old_name:
        return False, "Il nome è già quello attuale."
    if new_name in st.session_state.projects:
        return False, "Esiste già un progetto con questo nome."

    rebuilt = {}
    for key, value in st.session_state.projects.items():
        if key == old_name:
            value = copy.deepcopy(value)
            value.setdefault("meta", {})["name"] = new_name
            rebuilt[new_name] = value
        else:
            rebuilt[key] = value
    st.session_state.projects = rebuilt
...
def calc_urbanistica(p):
    ...
    # Premialità volumetriche ITACA / Decreto Romani.
    # La base è esplicita per evitare calcoli circolari o cumuli automatici non verificati.
    i = p.get("incentivi", {})
    ...
    base_premialita_nome = i.get("base_premialita", "Volume esistente legittimo")
    basi_premialita = {
        "Volume esistente legittimo": vol_esistente,
        "Indice urbanistico": vol_indice,
        "Titolo / progetto approvato": vol_approvato,
    ...
    base_incentivi = float(basi_premialita.get(base_premialita_nome, vol_esistente) or 0)
    itaca_incremento = base_incentivi * itaca / 100 if base_incentivi else 0.0
    romani_incremento = base_incentivi * romani / 100 if base_incentivi else 0.0
    vol_itaca = base_incentivi + itaca_incremento if base_incentivi else 0.0
    vol_romani = base_incentivi + romani_incremento if base_incentivi else 0.0
    vol_incentivato = base_incentivi + itaca_incremento + romani_incremento if base_incentivi else 0.0

    # Le fonti ITACA/Romani diventano selezionabili direttamente nella fattibilità.
    fonti.update({
        "Protocollo ITACA": vol_itaca,
        "Decreto Romani": vol_romani,
        "ITACA + Decreto Romani": vol_incentivato,
    })
    if not u.get("usa_volume_assunto_manuale", False) and fonte in fonti and fonte != "Confronto più regimi":
        vol_disponibile = float(fonti.get(fonte, 0) or 0)
        fonte_assunta = fonte
    ...
    return {
        ...
        "bonus_pct": bonus,
        "base_premialita_nome": base_premialita_nome,
        "base_premialita": base_incentivi,
        "itaca_pct": itaca,
        "romani_pct": romani,
        "itaca_incremento": itaca_incremento,
        "romani_incremento": romani_incremento,
        "vol_itaca": vol_itaca,
        "vol_romani": vol_romani,
...
def calc_sismabonus_acquisti_commerciale(p, scenario, ricavi_base):
    """
    Stima il vantaggio fiscale teorico degli acquirenti e, separatamente,
    l'eventuale quota che l'impresa ipotizza di capitalizzare nel prezzo.
    Il beneficio fiscale NON è di per sé un ricavo dell'impresa.
    """
    i = p.setdefault("incentivi", {})
    sb = i.setdefault("sismabonus_acquisti_commerciale", {})
    if not sb.get("attivo", False):
        return {
            "attivo": False, "unita": 0, "base_agevolata": 0.0, "aliquota_media": 0.0,
            "beneficio_acquirenti": 0.0, "premio_ricavi": 0.0,
            "ricavi_base": ricavi_base, "ricavi_valorizzati": ricavi_base
        }

    alloggi = max(1, int(p.get("programma", {}).get("alloggi", 1) or 1))
    richieste = int(sb.get("unita_agevolabili", 0) or 0)
    unita = min(alloggi, richieste) if richieste > 0 else alloggi
    cap = max(0.0, float(sb.get("massimale_per_unita_euro", 96000) or 0))

    # Prezzo medio: manuale se indicato, altrimenti ricavo medio per alloggio.
    prezzo_manuale = max(0.0, float(sb.get("prezzo_medio_unitario_manuale", 0) or 0))
    prezzo_medio = prezzo_manuale if prezzo_manuale > 0 else (ricavi_base / alloggi if alloggi else 0.0)
    base_per_unita = min(prezzo_medio, cap) if cap > 0 else prezzo_medio
    base_agevolata = base_per_unita * unita

    quota_pc = max(0.0, min(100.0, float(sb.get("quota_prima_casa_pct", 0) or 0))) / 100
    aliq_ord = float(sb.get("aliquota_ordinaria_pct", 36) or 0)
    aliq_pc = float(sb.get("aliquota_prima_casa_pct", 50) or 0)
    aliquota_media = aliq_ord + (aliq_pc - aliq_ord) * quota_pc

    beneficio = base_agevolata * aliquota_media / 100
    cattura = max(0.0, min(100.0, float(sb.get("quota_beneficio_capitalizzabile_pct", 0) or 0))) / 100
    premio = beneficio * cattura
    ricavi_valorizzati = ricavi_base + premio

    return {
        "attivo": True,
        "unita": unita,
        "prezzo_medio": prezzo_medio,
        "base_per_unita": base_per_unita,
        "base_agevolata": base_agevolata,
        "aliquota_media": aliquota_media,
        "beneficio_acquirenti": beneficio,
        "premio_ricavi": premio,
        "ricavi_base": ricavi_base,
        "ricavi_valorizzati": ricavi_valorizzati,
    }
...
def calc_project(p):
    ...
    for s, _ in SCENARIOS:
        ...
        sb_sales = calc_sismabonus_acquisti_commerciale(p, s, revenue_base)
        sb_cfg = p.get("incentivi", {}).get("sismabonus_acquisti_commerciale", {})
        if sb_sales["attivo"] and sb_cfg.get("includi_premio_nei_kpi", False):
            revenue = sb_sales["ricavi_valorizzati"]
        ...
        scenarios[s] = {
            ...
            "ricavi_base": revenue_base, "sismabonus_acquisti_commerciale": sb_sales,
        }
    return {"urbanistica": urb, "costi": cost, "scenari": scenarios}



def new_alternative_from_project(p, name="Nuova alternativa"):
    """Crea un'alternativa realmente editabile partendo dai dati commerciali del progetto corrente."""
    mix = copy.deepcopy(p.get("mix_prodotti", []))
    if not mix:
        pr = p.get("programma", {})
        alloggi = max(1, int(pr.get("alloggi", 1) or 1))
        sup_tot = float(pr.get("sup_commerciale_residenziale_mq", 0) or 0)
        sup_media = sup_tot / alloggi if alloggi else 0.0
        m = p.get("mercato", {})
        mix = [{
            "tipologia": "Alloggio tipo",
            "n": alloggi,
            "sup_media_mq": sup_media,
            "prezzo_prudente": sup_media * float(m.get("prezzo_mq_prudente", 0) or 0),
            "prezzo_probabile": sup_media * float(m.get("prezzo_mq_probabile", 0) or 0),
            "prezzo_ottimistico": sup_media * float(m.get("prezzo_mq_ottimistico", 0) or 0),
        }]
    return {
        "nome": name,
        "descrizione": "",
        "costo_sviluppo": float(calc_costs(p).get("sviluppo", 0) or 0),
        "mix_prodotti": mix,
    }


def unique_alternative_name(alts, base="Nuova alternativa"):
    if base not in alts:
        return base
    n = 2
    while f"{base} {n}" in alts:
        n += 1
    return f"{base} {n}"
...
def make_pdf(p, r):
    ...
    y = H - 55
    txt(45, y, "OLTREFORMA | FEASIBILITY V3.7", 15, True); y -= 24
    ...
    txt(55, y, f'ITACA {fmt(u["itaca_pct"])}% + Romani {fmt(u["romani_pct"])}% su {u["base_premialita_nome"]}: {fmt(u["vol_incentivato"],0)} m³ teorici'); y -= 24
    ...
    c.drawString(45, 28, "Oltreforma | Feasibility V3.7 · Documento preliminare, non sostitutivo di perizia o verifica urbanistica/fiscale.")
...
with st.sidebar:
    ...
    st.markdown("## Feasibility V3.7")
    st.caption("Fattibilità immobiliare · Studio / Impresa")

    names = list(st.session_state.projects.keys())
    if names:
        # Se il progetto corrente non esiste più, usa il primo solo come recupero eccezionale.
        if st.session_state.current_project not in names:
            st.session_state.current_project = names[0]
        if st.session_state.get("project_selector") not in names:
            st.session_state.project_selector = st.session_state.current_project
        ...
        st.selectbox(
            ...
            on_change=select_current_project,
            help="Il progetto selezionato resta quello attivo anche quando modifichi altri campi dell'app."
        )
        st.caption(f'Attivo: **{st.session_state.current_project}**')

    st.session_state.view_mode = st.radio(
        "Vista",
        ["Vista Studio", "Vista Cliente / Impresa"],
        index=0 if st.session_state.get("view_mode", "Vista Studio") == "Vista Studio" else 1,
        horizontal=False
    )
    ...
    with st.expander("Rinomina progetto", expanded=False):
        current_name = st.session_state.current_project or ""
        rename_value = st.text_input(
            "Nuovo nome",
            value=current_name,
            key=f"rename_project_{slug(current_name)}"
        )
        if st.button("Rinomina", use_container_width=True):
            ok, msg = rename_current_project(rename_value)
            if ok:
                st.rerun()
            else:
                st.warning(msg)

    with st.expander("Crea / duplica", expanded=False):
        new_name = st.text_input("Nome nuovo progetto", placeholder="Es. Area B3 - Via Toscani", key="new_project_name")
        st.caption("Per ora il nuovo progetto usa quello aperto come modello, così non perdi la struttura dei parametri.")
        if st.button("＋ Crea da progetto aperto", use_container_width=True):
            name = new_name.strip()
            if not name:
                st.warning("Inserisci il nome del nuovo progetto.")
            elif name in st.session_state.projects:
                st.warning("Esiste già un progetto con questo nome.")
            elif st.session_state.current_project:
                template = copy.deepcopy(get_project())
                template.setdefault("meta", {})["name"] = name
                template["meta"]["indirizzo"] = name
                st.session_state.projects[name] = template
                st.session_state.current_project = name
                st.session_state.project_selector = name
                clear_project_widget_state()
                save_projects(st.session_state.projects)
                st.rerun()

        if st.button("Duplica progetto aperto", use_container_width=True) and st.session_state.current_project:
            ...
            st.session_state.projects[n].setdefault("meta", {})["name"] = n
            st.session_state.current_project = n
            st.session_state.project_selector = n
            clear_project_widget_state()
            save_projects(st.session_state.projects)
            st.rerun()
...
# ---------- Studio tabs ----------
tabs = st.tabs(["Dashboard", "Immobile", "Urbanistica", "Product Mix", "Costi", "Mercato", "Alternative", "Sismabonus", "Detrazioni 2026", "Acquisizione / Permuta", "Onorario Studio", "Report", "Guida / Legenda"])
...
with tabs[0]:
    ...
    sb_dash = rr_dash["scenari"]["probabile"].get("sismabonus_acquisti_commerciale", {})
    if sb_dash.get("attivo"):
        st.markdown("#### Sismabonus acquisti · scenario commerciale")
        sba, sbb, sbc_col = st.columns(3)
        sba.metric("Ricavi base", euro(sb_dash.get("ricavi_base", 0)))
        sbb.metric("Beneficio teorico acquirenti", euro(sb_dash.get("beneficio_acquirenti", 0)))
        sbc_col.metric("Premio ricavi ipotizzato", euro(sb_dash.get("premio_ricavi", 0)))
        st.caption("Il beneficio fiscale appartiene all'acquirente; nei ricavi entra soltanto l'eventuale maggior prezzo commerciale ipotizzato.")
...
with tabs[1]:
    ...
    with c1:
        st.text_input(
            ...
            disabled=True,
            help="Per rinominare il progetto usa Gestione progetti → Rinomina progetto nella barra laterale."
        )
        p["meta"]["name"] = st.session_state.current_project
...
with tabs[2]:
    ...
    with st.expander("2 · Premialità volumetriche · Piano Casa / ITACA / Decreto Romani", expanded=True):
        pc1, pc2 = st.columns(2)
        ...
        st.divider()
        st.markdown("##### Protocollo ITACA")
        i = p.setdefault("incentivi", {})
        it1, it2 = st.columns([1, 1])
        i["itaca_attivo"] = it1.checkbox(
            "Attiva premialità ITACA",
            value=bool(i.get("itaca_attivo", False)),
            help="Attiva il calcolo volumetrico teorico. L'effettiva applicabilità va verificata sul caso concreto."
        )
        i["itaca_pct"] = it2.number_input(
            "Incremento ITACA (%)", 0.0,
            value=float(i.get("itaca_pct", 0)), step=.5
        )
        it_opts = ["Verificato", "Da verificare oltre densità ordinaria", "Da verificare", "Non applicabile"]
        cur_it = i.get("itaca_stato", "Da verificare")
        i["itaca_stato"] = st.selectbox(
            "Stato verifica ITACA", it_opts,
            index=it_opts.index(cur_it) if cur_it in it_opts else 2
        )
        i["itaca_riferimento"] = st.text_input(
            "Norma / delibera / riferimento ITACA",
            i.get("itaca_riferimento", ""),
            placeholder="Es. L.R., delibera comunale, Protocollo ITACA, articolo..."
        )

        st.divider()
        st.markdown("##### Decreto Romani")
        ro1, ro2 = st.columns([1, 1])
        i["romani_attivo"] = ro1.checkbox(
            "Attiva premialità Decreto Romani",
            value=bool(i.get("romani_attivo", False)),
            help="Attiva il calcolo volumetrico teorico. Applicabilità e cumulabilità vanno verificate sul caso concreto."
        )
        i["romani_pct"] = ro2.number_input(
            "Incremento Decreto Romani (%)", 0.0,
            value=float(i.get("romani_pct", 0)), step=.5
        )
        ro_opts = ["Verificato", "Da verificare cumulabilità/applicabilità", "Da verificare", "Non applicabile"]
        cur_ro = i.get("romani_stato", "Da verificare")
        i["romani_stato"] = st.selectbox(
            "Stato verifica Decreto Romani", ro_opts,
            index=ro_opts.index(cur_ro) if cur_ro in ro_opts else 2
        )
        i["romani_riferimento"] = st.text_input(
            "Norma / delibera / riferimento Decreto Romani",
            i.get("romani_riferimento", ""),
            placeholder="Es. D.Lgs., articolo, recepimento / riferimento locale..."
        )

        st.divider()
        st.markdown("##### Base e cumulabilità")
        base_opts = [
        ...
        cur_base = i.get("base_premialita", "Volume esistente legittimo")
        i["base_premialita"] = st.selectbox(
            "Base sulla quale calcolare ITACA e Decreto Romani",
            base_opts,
            index=base_opts.index(cur_base) if cur_base in base_opts else 0,
            help="La base è scelta esplicitamente: l'app non presume automaticamente quale volume sia giuridicamente utilizzabile."
        )
        i["cumulabilita_itaca_romani_verificata"] = st.checkbox(
            "Cumulabilità ITACA + Decreto Romani verificata",
            value=bool(i.get("cumulabilita_itaca_romani_verificata", False))
        )
        if i.get("itaca_attivo", False) and i.get("romani_attivo", False) and not i.get("cumulabilita_itaca_romani_verificata", False):
            st.warning("ITACA e Decreto Romani sono entrambi attivi, ma la cumulabilità non è verificata: il totale combinato resta solo uno scenario teorico.")
    ...
    rows_urb = [
        ...
        {"Fonte": "Protocollo ITACA", "Volume": f'{fmt(ur["vol_itaca"],0)} m³' if p["incentivi"].get("itaca_attivo",False) else "—", "Stato": p["incentivi"].get("itaca_stato","Da verificare")},
        {"Fonte": "Decreto Romani", "Volume": f'{fmt(ur["vol_romani"],0)} m³' if p["incentivi"].get("romani_attivo",False) else "—", "Stato": p["incentivi"].get("romani_stato","Da verificare")},
        {"Fonte": "ITACA + Decreto Romani", "Volume": f'{fmt(ur["vol_incentivato"],0)} m³' if (p["incentivi"].get("itaca_attivo",False) or p["incentivi"].get("romani_attivo",False)) else "—", "Stato": "Cumulabilità verificata" if p["incentivi"].get("cumulabilita_itaca_romani_verificata",False) else "Scenario teorico"},
    ...
    with st.expander("Come sono ottenuti questi risultati?", expanded=True):
        ...
        st.markdown("**Protocollo ITACA / Decreto Romani**")
        st.write(f'Base scelta: **{ur["base_premialita_nome"]} = {fmt(ur["base_premialita"],0)} m³**')
        if p["incentivi"].get("itaca_attivo", False):
            st.write(f'ITACA: {fmt(ur["base_premialita"],0)} × {fmt(ur["itaca_pct"],2)}% = +{fmt(ur["itaca_incremento"],0)} m³ → **{fmt(ur["vol_itaca"],0)} m³**')
        if p["incentivi"].get("romani_attivo", False):
            st.write(f'Decreto Romani: {fmt(ur["base_premialita"],0)} × {fmt(ur["romani_pct"],2)}% = +{fmt(ur["romani_incremento"],0)} m³ → **{fmt(ur["vol_romani"],0)} m³**')
        if p["incentivi"].get("itaca_attivo", False) and p["incentivi"].get("romani_attivo", False):
            st.write(f'Combinazione teorica: {fmt(ur["base_premialita"],0)} + {fmt(ur["itaca_incremento"],0)} + {fmt(ur["romani_incremento"],0)} = **{fmt(ur["vol_incentivato"],0)} m³**')
            if not p["incentivi"].get("cumulabilita_itaca_romani_verificata", False):
                st.caption("La combinazione non va assunta come diritto edificatorio finché la cumulabilità non è verificata.")
...
with tabs[3]:
    ...
    if p["analisi"]["modalita_ricavi"] == "Sintetico €/m²":
        pr = p["programma"]
        st.info("In questa modalità le superfici inserite qui sotto sono **TOTALI dell'intero intervento**, non del singolo appartamento.")

        c1, c2, c3 = st.columns(3)
        pr["alloggi"] = c1.number_input(
            "Numero totale alloggi",
            1, value=int(pr.get("alloggi", 1)),
            help="Numero complessivo di unità residenziali previste."
        )
        pr["sup_commerciale_residenziale_mq"] = c2.number_input(
            "Superficie commerciale residenziale TOTALE (m²)",
            0.0, value=float(pr.get("sup_commerciale_residenziale_mq", 0)), step=1.0,
            help="Somma della superficie commerciale residenziale di tutti gli alloggi. Non è la superficie del singolo appartamento."
        )
        pr["box"] = c3.number_input(
            "Numero totale box",
            0, value=int(pr.get("box", 0)),
            help="Numero complessivo dei box."
        )

        d1, d2 = st.columns(2)
        pr["balconi_fisici_mq"] = d1.number_input(
            "Superficie balconi TOTALE (m²)",
            0.0, value=float(pr.get("balconi_fisici_mq", 0)), step=1.0,
            help="Somma delle superfici fisiche di tutti i balconi dell'intervento."
        )
        pr["box_mq_totali"] = d2.number_input(
            "Superficie box TOTALE (m²)",
            0.0, value=float(pr.get("box_mq_totali", 0)), step=1.0,
            help="Somma delle superfici di tutti i box."
        )

        alloggi = max(1, int(pr.get("alloggi", 1)))
        box_n = int(pr.get("box", 0))
        m1, m2, m3 = st.columns(3)
        m1.metric(
            "Superficie media residenziale / alloggio",
            f'{fmt(float(pr.get("sup_commerciale_residenziale_mq",0))/alloggi,1)} m²'
        )
        m2.metric(
            "Balconi medi / alloggio",
            f'{fmt(float(pr.get("balconi_fisici_mq",0))/alloggi,1)} m²'
        )
        m3.metric(
            "Superficie media / box",
            f'{fmt(float(pr.get("box_mq_totali",0))/box_n,1)} m²' if box_n > 0 else "—"
        )
        st.caption("Le medie sono solo un controllo rapido. Se gli appartamenti hanno metrature diverse usa **Mix per tipologia**.")
    else:
        ...
        for idx, row in enumerate(list(mix)):
            with st.container(border=True):
                ...
                row["sup_media_mq"] = c3.number_input("Sup. media SINGOLA unità (m²)", 0.0, value=float(row.get("sup_media_mq", 0)), step=5.0, key=f"mix_sup_{idx}")
...
with tabs[6]:
    ...
    st.caption(
        "Qui puoi creare e modificare vere ipotesi alternative senza cambiare il progetto principale: "
        "numero e dimensione degli alloggi, prezzi di vendita e costo di sviluppo."
    )

    alts = p.setdefault("alternative", {})

    h1, h2 = st.columns([2, 1])
    new_alt_name = h1.text_input(
        "Nome nuova alternativa",
        placeholder="Es. 8 alloggi più grandi / 10 alloggi più piccoli",
        key="new_alt_name"
    )
    if h2.button("＋ Crea alternativa", use_container_width=True):
        proposed = (new_alt_name or "").strip() or unique_alternative_name(alts)
        if proposed in alts:
            st.warning("Esiste già un'alternativa con questo nome.")
        else:
            alts[proposed] = new_alternative_from_project(p, proposed)
            st.rerun()

    if not alts:
        st.info(
            "Non ci sono ancora alternative. Scrivi un nome e premi **Crea alternativa**. "
            "La nuova ipotesi partirà dai dati del progetto corrente, ma potrai modificarla liberamente."
        )

    delete_name = None
    rename_action = None

    for alt_index, (name, alt) in enumerate(list(alts.items())):
        alt.setdefault("nome", name)
        alt.setdefault("descrizione", "")
        alt.setdefault("costo_sviluppo", float(calc_costs(p).get("sviluppo", 0) or 0))
        alt.setdefault("mix_prodotti", [])

        with st.expander(f"Alternativa · {name}", expanded=True):
            top1, top2 = st.columns([2, 1])
            alt["descrizione"] = top1.text_input(
                "Descrizione / logica dell'alternativa",
                value=alt.get("descrizione", ""),
                placeholder="Es. riduco il numero di alloggi e aumento la metratura media",
                key=f"alt_desc_{alt_index}_{slug(name)}"
            )
            alt["costo_sviluppo"] = top2.number_input(
                "Costo sviluppo alternativa (€)",
                0.0,
                value=float(alt.get("costo_sviluppo", 0)),
                step=10000.0,
                key=f"altcost_{alt_index}_{slug(name)}"
            )

            st.markdown("#### Mix dell'alternativa")
            st.caption(
                "Ogni riga rappresenta una tipologia. La superficie è quella della **SINGOLA unità**; "
                "i prezzi sono il prezzo di vendita della singola unità nei tre scenari."
            )

            remove_row = None
            for j, row in enumerate(alt.get("mix_prodotti", [])):
                row.setdefault("tipologia", f"Tipologia {j+1}")
                row.setdefault("n", 1)
                row.setdefault("sup_media_mq", 0.0)
                row.setdefault("prezzo_prudente", 0.0)
                row.setdefault("prezzo_probabile", 0.0)
                row.setdefault("prezzo_ottimistico", 0.0)

                st.markdown(f"**Tipologia {j+1}**")
                c1, c2, c3 = st.columns([1.7, .7, 1])
                row["tipologia"] = c1.text_input(
                    "Nome tipologia",
                    value=row.get("tipologia", ""),
                    key=f"alt_type_{alt_index}_{j}_{slug(name)}"
                )
                row["n"] = c2.number_input(
                    "N. unità",
                    min_value=0,
                    value=int(row.get("n", 0)),
                    step=1,
                    key=f"alt_n_{alt_index}_{j}_{slug(name)}"
                )
                row["sup_media_mq"] = c3.number_input(
                    "Sup. SINGOLA unità (m²)",
                    min_value=0.0,
                    value=float(row.get("sup_media_mq", 0)),
                    step=1.0,
                    key=f"alt_sup_{alt_index}_{j}_{slug(name)}"
                )

                p1, p2, p3, p4 = st.columns([1, 1, 1, .45])
                row["prezzo_prudente"] = p1.number_input(
                    "Prezzo unità · Prudente (€)",
                    min_value=0.0,
                    value=float(row.get("prezzo_prudente", 0)),
                    step=5000.0,
                    key=f"alt_pp_{alt_index}_{j}_{slug(name)}"
                )
                row["prezzo_probabile"] = p2.number_input(
                    "Prezzo unità · Probabile (€)",
                    min_value=0.0,
                    value=float(row.get("prezzo_probabile", 0)),
                    step=5000.0,
                    key=f"alt_pb_{alt_index}_{j}_{slug(name)}"
                )
                row["prezzo_ottimistico"] = p3.number_input(
                    "Prezzo unità · Ottimistico (€)",
                    min_value=0.0,
                    value=float(row.get("prezzo_ottimistico", 0)),
                    step=5000.0,
                    key=f"alt_po_{alt_index}_{j}_{slug(name)}"
                )
                if p4.button("🗑", key=f"alt_delrow_{alt_index}_{j}_{slug(name)}", help="Elimina questa tipologia"):
                    remove_row = j

                st.caption(
                    f'Totale superficie tipologia: **{fmt(int(row.get("n",0)) * float(row.get("sup_media_mq",0)),1)} m²**'
                )
                st.divider()

            if remove_row is not None:
                alt["mix_prodotti"].pop(remove_row)
                st.rerun()

            if st.button("＋ Aggiungi tipologia", key=f"alt_addrow_{alt_index}_{slug(name)}"):
                alt["mix_prodotti"].append({
                    "tipologia": f"Nuova tipologia {len(alt['mix_prodotti'])+1}",
                    "n": 1,
                    "sup_media_mq": 0.0,
                    "prezzo_prudente": 0.0,
                    "prezzo_probabile": 0.0,
                    "prezzo_ottimistico": 0.0,
                })
                st.rerun()

            ar = calc_alternative(alt, float(p["acquisizione"].get("target_margin_pct", 18)))
            total_units_alt = sum(int(x.get("n",0)) for x in alt.get("mix_prodotti", []))
            total_area_alt = sum(int(x.get("n",0)) * float(x.get("sup_media_mq",0)) for x in alt.get("mix_prodotti", []))

            st.markdown("#### Risultato dell'alternativa")
            q1, q2, q3 = st.columns(3)
            q1.metric("Unità totali", total_units_alt)
            q2.metric("Superficie residenziale totale", f"{fmt(total_area_alt,1)} m²")
            q3.metric("Costo sviluppo", euro(alt.get("costo_sviluppo",0)))
            ...
            for s, label in SCENARIOS:
                ...
                rows.append({
                    "Scenario": label,
                    "Ricavi": euro(x["ricavi"]),
                    "Costo sviluppo": euro(x["costo"]),
                    "Residuo prima del suolo": euro(x["residuo"]),
                    "Margine prima del suolo": f'{fmt(x["margine_pre_land"],1)}%',
                    "Max suolo/permuta al target": euro(x["max_land"])
                })
            st.dataframe(rows, hide_index=True, use_container_width=True)

            st.markdown("#### Gestione alternativa")
            g1, g2, g3 = st.columns([2, .8, .8])
            rename_value = g1.text_input(
                "Rinomina alternativa",
                value=name,
                key=f"alt_rename_{alt_index}_{slug(name)}"
            )
            if g2.button("Rinomina", key=f"alt_rename_btn_{alt_index}_{slug(name)}", use_container_width=True):
                rename_action = (name, rename_value.strip())
            if g3.button("Elimina", key=f"alt_delete_{alt_index}_{slug(name)}", use_container_width=True):
                delete_name = name

    if rename_action:
        old_name, new_name = rename_action
        if not new_name:
            st.warning("Il nome dell'alternativa non può essere vuoto.")
        elif new_name != old_name and new_name in alts:
            st.warning("Esiste già un'alternativa con questo nome.")
            ...
            for k, v in alts.items():
                if k == old_name:
                    ...
                    rebuilt[k] = v
            p["alternative"] = rebuilt
            st.rerun()

    if delete_name:
        del alts[delete_name]
        st.rerun()

with tabs[7]:
    st.markdown("### Sismabonus · screening")
    st.caption("Le premialità volumetriche ITACA e Decreto Romani sono ora nella scheda Urbanistica, accanto al Piano Casa.")
    i = p["incentivi"]
    if p["meta"].get("tipologia_intervento") == "Demolizione + ricostruzione":
        ...
        sb_opts = ["Da verificare requisiti fiscali e tecnici", "Potenzialmente applicabile", "Verificato", "Non applicabile"]
        cur_sb = i.get("sismabonus_stato", sb_opts[0])
        i["sismabonus_stato"] = st.selectbox(
            "Stato verifica Sismabonus",
            sb_opts,
            index=sb_opts.index(cur_sb) if cur_sb in sb_opts else 0
        )
        ...
        st.info("Lo screening Sismabonus dedicato alla demolizione + ricostruzione si attiva scegliendo quella tipologia di intervento nella scheda Immobile.")

    st.divider()
    st.markdown("### Sismabonus acquisti · alternativa commerciale nei ricavi")
    st.caption(
        "Il Sismabonus acquisti è una detrazione dell'acquirente. Qui puoi però misurarlo come "
        "**leva commerciale** e simulare quanta parte del vantaggio fiscale possa tradursi in un maggior prezzo di vendita."
    )

    sbc = i.setdefault("sismabonus_acquisti_commerciale", {})
    sbc["attivo"] = st.checkbox(
        "Valuta il Sismabonus acquisti come alternativa commerciale",
        value=bool(sbc.get("attivo", False)),
        key="sbacq_comm_attivo"
    )

    if sbc["attivo"]:
        if p["meta"].get("tipologia_intervento") != "Demolizione + ricostruzione":
            st.warning("Questa simulazione è pensata per demolizione e ricostruzione di interi edifici da parte di impresa. Verifica la qualificazione edilizia del caso.")

        c1, c2, c3 = st.columns(3)
        sbc["zona_sismica"] = c1.selectbox(
            "Zona sismica",
            [0, 1, 2, 3, 4],
            index=[0,1,2,3,4].index(int(sbc.get("zona_sismica",0) or 0)) if int(sbc.get("zona_sismica",0) or 0) in [0,1,2,3,4] else 0,
            format_func=lambda x: "Da verificare" if x == 0 else f"Zona {x}",
            key="sbacq_comm_zona"
        )
        sbc["unita_agevolabili"] = c2.number_input(
            "Unità agevolabili · 0 = tutti gli alloggi",
            min_value=0,
            value=int(sbc.get("unita_agevolabili", 0) or 0),
            step=1,
            key="sbacq_comm_unita"
        )
        sbc["massimale_per_unita_euro"] = c3.number_input(
            "Massimale base per unità (€)",
            min_value=0.0,
            value=float(sbc.get("massimale_per_unita_euro", 96000) or 0),
            step=1000.0,
            key="sbacq_comm_cap"
        )

        d1, d2, d3 = st.columns(3)
        sbc["aliquota_ordinaria_pct"] = d1.number_input(
            "Aliquota ordinaria 2026 (%)",
            min_value=0.0, max_value=100.0,
            value=float(sbc.get("aliquota_ordinaria_pct", 36) or 0),
            step=1.0,
            key="sbacq_comm_ord"
        )
        sbc["aliquota_prima_casa_pct"] = d2.number_input(
            "Aliquota abitazione principale 2026 (%)",
            min_value=0.0, max_value=100.0,
            value=float(sbc.get("aliquota_prima_casa_pct", 50) or 0),
            step=1.0,
            key="sbacq_comm_pc"
        )
        sbc["quota_prima_casa_pct"] = d3.number_input(
            "Vendite stimate come abitazione principale (%)",
            min_value=0.0, max_value=100.0,
            value=float(sbc.get("quota_prima_casa_pct", 0) or 0),
            step=5.0,
            key="sbacq_comm_quota_pc"
        )

        sbc["prezzo_medio_unitario_manuale"] = st.number_input(
            "Prezzo medio unitario manuale (€) · 0 = calcolo automatico dai ricavi",
            min_value=0.0,
            value=float(sbc.get("prezzo_medio_unitario_manuale", 0) or 0),
            step=5000.0,
            key="sbacq_comm_prezzo"
        )

        st.markdown("#### Quanto del vantaggio fiscale può diventare maggior ricavo?")
        sbc["quota_beneficio_capitalizzabile_pct"] = st.slider(
            "Quota del beneficio teorico capitalizzabile nel prezzo di vendita (%)",
            min_value=0, max_value=100,
            value=int(round(float(sbc.get("quota_beneficio_capitalizzabile_pct", 0) or 0))),
            step=5,
            key="sbacq_comm_capture",
            help="È un'ipotesi commerciale, non una detrazione spettante all'impresa. 0% = nessun aumento ricavi; 100% = si ipotizza di trasferire integralmente il vantaggio fiscale nel prezzo."
        )

        v1, v2 = st.columns(2)
        sbc["requisiti_verificati"] = v1.checkbox(
            "Requisiti tecnici/fiscali verificati",
            value=bool(sbc.get("requisiti_verificati", False)),
            key="sbacq_comm_req"
        )
        sbc["vendita_entro_30_mesi"] = v2.checkbox(
            "Alienazione prevista entro 30 mesi dalla fine lavori",
            value=bool(sbc.get("vendita_entro_30_mesi", False)),
            key="sbacq_comm_30m"
        )

        base_prob = calc_mix_revenue(p.get("mix_prodotti", []), "probabile")[0] if (
            p.get("analisi", {}).get("modalita_ricavi") == "Mix per tipologia" and p.get("mix_prodotti")
        ) else calc_synthetic_revenue(p, "probabile")
        sb_calc = calc_sismabonus_acquisti_commerciale(p, "probabile", base_prob)

        s1, s2, s3, s4 = st.columns(4)
        s1.metric("Ricavi base", euro(sb_calc["ricavi_base"]))
        s2.metric("Beneficio teorico acquirenti", euro(sb_calc["beneficio_acquirenti"]))
        s3.metric("Premio ricavi ipotizzato", euro(sb_calc["premio_ricavi"]))
        s4.metric("Ricavi scenario Sismabonus", euro(sb_calc["ricavi_valorizzati"]))

        with st.expander("Come viene calcolato?", expanded=False):
            st.write(f'**Unità considerate:** {sb_calc["unita"]}')
            st.write(f'**Prezzo medio unitario usato:** {euro(sb_calc.get("prezzo_medio",0))}')
            st.write(f'**Base agevolata per unità:** {euro(sb_calc.get("base_per_unita",0))}')
            st.write(f'**Base agevolata complessiva:** {euro(sb_calc["base_agevolata"])}')
            st.write(f'**Aliquota media stimata:** {fmt(sb_calc["aliquota_media"],1)}%')
            st.write(f'**Beneficio acquirenti:** {euro(sb_calc["base_agevolata"])} × {fmt(sb_calc["aliquota_media"],1)}% = {euro(sb_calc["beneficio_acquirenti"])}')
            st.write(f'**Premio commerciale:** {euro(sb_calc["beneficio_acquirenti"])} × {fmt(sbc["quota_beneficio_capitalizzabile_pct"],0)}% = {euro(sb_calc["premio_ricavi"])}')

        sbc["includi_premio_nei_kpi"] = st.checkbox(
            "Usa lo scenario Sismabonus nei KPI economici principali",
            value=bool(sbc.get("includi_premio_nei_kpi", False)),
            key="sbacq_comm_kpi",
            help="Se attivo, ai ricavi base viene aggiunto SOLO il premio commerciale ipotizzato, non l'intera detrazione dell'acquirente."
        )

        if sbc["zona_sismica"] not in [1, 2, 3]:
            st.warning("Per il Sismabonus acquisti va verificata l'ubicazione in zona sismica 1, 2 o 3.")
        if not sbc["vendita_entro_30_mesi"]:
            st.warning("Verificare il requisito temporale di alienazione da parte dell'impresa entro 30 mesi dalla conclusione dei lavori.")
        if not sbc["requisiti_verificati"]:
            st.warning("Scenario ancora preliminare: requisiti tecnici e fiscali non marcati come verificati.")

        st.info(
            "Importante: il beneficio fiscale resta dell'acquirente. L'eventuale 'premio ricavi' è una tua ipotesi di "
            "pricing/valorizzazione commerciale e va tenuta distinta contabilmente dal Sismabonus."
        )
...
- **Piano Casa / premialità** = scenario teorico calcolato sul volume esistente. **Protocollo ITACA** e **Decreto Romani** sono gestiti nella stessa scheda Urbanistica con base di calcolo esplicita, stato di verifica e controllo della cumulabilità. Nessuna premialità va assunta come diritto edificatorio senza verifica del caso concreto.
...
""")
    ...
    st.markdown("#### Product Mix")
    st.markdown("""
- In **Sintetico €/m²**, la **superficie commerciale residenziale**, i **balconi** e i **box** sono sempre valori **TOTALI dell'intero intervento**.
- L'app mostra automaticamente la superficie media per alloggio e, se presenti, balconi medi e superficie media dei box.
- In **Mix per tipologia**, invece, la superficie inserita nella singola riga è la **superficie media della singola unità** di quella tipologia; il totale viene calcolato come `numero unità × superficie media`.
""")
...
""")
    ...
    st.markdown("#### Alternative progettuali")
    st.markdown("""
- Ogni alternativa è indipendente dal progetto principale e può avere un proprio **mix di tipologie**, numero di unità, superficie della singola unità, prezzi di vendita e costo sviluppo.
- **Superficie totale alternativa** = somma di `numero unità × superficie singola unità`.
- I ricavi vengono ricalcolati per gli scenari prudente, probabile e ottimistico.
- **Max suolo/permuta al target** indica quanto può essere riconosciuto al proprietario mantenendo il margine obiettivo del progetto.
""")
...
- Per il **Sismabonus acquisti** la scheda dedicata consente anche uno **scenario commerciale**: l'app calcola il beneficio teorico dell'acquirente e permette di ipotizzare quale quota possa essere capitalizzata nel prezzo di vendita.
- Solo questa eventuale **quota di maggior prezzo** può essere aggiunta ai ricavi nello scenario economico; la detrazione fiscale in sé resta dell'acquirente.
